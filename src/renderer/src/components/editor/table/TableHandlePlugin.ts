...
import { createTableDragImage } from './createDragImage'
...
// ---------------------------------------------------------------------------
// TableHandleView — ProseMirror plugin view
// ---------------------------------------------------------------------------
...
class TableHandleView implements PluginView {
  ...
  private _handleMouseMoveNow(event: MouseEvent) {
    ...
    let blockInfo: { node: PMNode; pos: number } | undefined
    for (let d = $pos.depth; d >= 0; d--) {
      const node = $pos.node(d)
      if (isTableNode(node)) {
  ...
  update(view: EditorView): void {
    ...
    // Compare rect VALUES, not object identity — getBoundingClientRect returns
    // a fresh DOMRect every call, so `!==` was always true and every keystroke
    // re-emitted (→ TableHandle/TableExtendButtons re-render + reflow), which
    // is what made typing in a table lag. Now we only emit when the handle
    // actually needs to move (e.g. a cell grew and wrapped).
    const refPosChanged =
      !rectEq(newReferencePosCell ?? null, this.state.referencePosCell ?? null) ||
      !rectEq(newReferencePosTable, this.state.referencePosTable)
  ...
  destroy(): void {
    ...
    // Drop the module-level reference so drag handlers don't act on a
    // destroyed view (guard so a freshly created view isn't nulled out).
    if (tableHandleView === this) tableHandleView = null
  }
}

// ---------------------------------------------------------------------------
// Module-level reference for drag start/end
// ---------------------------------------------------------------------------
...
function TableHandlePluginFactory(
  editor: Editor,
  emitUpdate: (state: TableHandlesState) => void,
): Plugin {
  return new Plugin({
    ...
    props: {
      decorations: (state) => {
        if (!tableHandleView?.state?.draggingState || tableHandleView.tablePos === undefined) {
          return null
        }
        ...
        const sourceCells =
          draggingState.draggedCellOrientation === 'row'
            ? getRowCells(editor, originalIndex, tableHandleView.state.blockPos)
            : getColumnCells(editor, originalIndex, tableHandleView.state.blockPos)

        sourceCells.cells.forEach((cell) => {
          if (cell.node) {
            decorations.push(
        ...
        const targetCells = isRow
          ? getRowCells(editor, newIndex, tableHandleView.state.blockPos)
          : getColumnCells(editor, newIndex, tableHandleView.state.blockPos)

        targetCells.cells.forEach((cell) => {
...
export const dragEnd = () => {
  ...
  const { editor } = tableHandleView
  editor.view.dispatch(editor.state.tr.setMeta(tableHandlePluginKey, null))
}

// ---------------------------------------------------------------------------
// TableHandleExtension — Tiptap extension that wires up the plugin
// ---------------------------------------------------------------------------
...
export const TableHandleExtension = Extension.create({
  ...
  addProseMirrorPlugins() {
    const { editor } = this
    return [
      TableHandlePluginFactory(editor, (state) => {
